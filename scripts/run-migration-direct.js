 ...
 * This bypasses any connection pooling issues
 */
...
// Get database URL and convert to direct connection if using PgBouncer
...
// If using PgBouncer, we need to use the direct connection
// Supabase direct connection format: postgresql://user:pass@db.xxx.supabase.co:5432/postgres
if (dbUrl.includes('pooler.supabase.com')) {
  ...
  if (match) {
    ...
    const directHost = host.replace('pooler.supabase.com:6543', 'supabase.co:5432');
    dbUrl = `postgresql://${user}:${pass}@${directHost}/${database}`;
    console.log('🔄 Using direct connection (bypassing PgBouncer)');
  }
}
